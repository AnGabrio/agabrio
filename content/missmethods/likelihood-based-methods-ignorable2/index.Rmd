...
\\[
p(y \\mid \\mu)=\\frac{1}{\\sqrt{\\left(2\\pi\\sigma\^2\\right)\^n}}\\text{exp}\\left(-\\frac{1}{2}\\sum\_{i=1}\^n \\frac{(y\_i-\\mu)\^2}{\\sigma\^2} \\right),
\\]
...
\\[
p(\\mu) = \\frac{1}{\\sqrt{2\\pi\\sigma\^2\_0}}\\text{exp}\\left(-\\frac{1}{2}\\frac{(\\mu -\\mu\_0)\^2}{\\sigma\^2} \\right),
\\]
...
\\[
p(\\mu \\mid y) = \\frac{p(\\mu)p(y\\mid \\mu)}{p(y)} \\propto \\frac{1}{\\sqrt{2\\pi\\sigma\^2\_0}}\\frac{1}{\\sqrt{\\left(2\\pi\\sigma\^2\\right)\^n}}\\text{exp}\\left(-\\frac{1}{2} \\left[\\frac{(\\mu - \\mu\_0)\^2}{\\sigma\^2\_0} + \\sum\_{i=1}\^n\\frac{(y\_i-\\mu)\^2}{\\sigma\^2} \\right] \\right).
\\]

Exapanding the components, collecting terms and completing the square in $\mu$ gives

\\[
p(\\mu \\mid y) \\propto \\text{exp}\\left(-\\frac{(\\mu - \\mu\_1)}{2\\tau\^2\_1} \\right),
\\]

that is the posterior distribution of $\mu$ given $y$ is Normal with posterior mean $\mu_1$ and variance $\tau^2_1$, where

\\[
\\mu\_1 = \\frac{\\frac{1}{\\tau\^2\_0}\\mu\_0 + \\frac{n}{\\sigma\^2}\\bar{y}}{\\frac{1}{\\tau\^2\_0} + \\frac{n}{\\sigma\^2}} \\;\\;\\; \\text{and} \\;\\;\\; \\frac{1}{\\tau\^2\_1}=\\frac{1}{\\tau\^2\_0} + \\frac{n}{\\sigma\^2}.
\\]
...
\\[
p(y \\mid \\sigma\^2)=\\frac{1}{\\sqrt{\\left(2\\pi\\sigma\^2\\right)\^n}}\\text{exp}\\left(-\\frac{1}{2}\\sum\_{i=1}\^n \\frac{(y\_i-\\mu)\^2}{\\sigma\^2} \\right),
\\]
...
\\[
p(\\mu \\mid y)=\\int\_{0}^{\\infty}p(\\mu,\\sigma\^2\\mid y)d\\sigma\^2 \\propto \\left[1+\\frac{n(\\mu-\\bar{y})}{(n-1)s\^2} \\right]
\\]

which corresponds to a Student-$t$ density with $n-1$ degrees of freedom

\\[
p(\\mu \\mid y)\\sim t\_{n-1}\\left(\\bar{y},\\frac{s\^2}{n}\\right)
\\]
...
\\[
p(y\\mid \\mu, \\Sigma) \\propto \\mid \\Sigma \\mid\^{-n/2}\\text{exp}\\left(-\\frac{1}{2}\\sum\_{i=1}\^n(y\_i-\\mu)\^{T}\\Sigma\^{-1}(y\_i-\\mu) \\right),
\\]
...
\\[
p(\\mu \\mid y) \\sim N(\\mu\_1,\\Sigma\_1),
\\]
...
\\[
p(\\mu \\mid \\Sigma, y) \\sim N(\\mu\_1,\\Sigma\_1) \\;\\;\\; \\text{and} \\;\\;\\; p(\\Sigma \\mid y) \\sim \\text{Inv-Wishart}(\\Lambda\_1,\\nu\_1), 
\\]
...
\\[
p(\\beta \\mid \\sigma\^2, y) \\sim N(\\hat{\\beta},V\_{\\beta}\\sigma\^2),
\\]
...
\\[
p(y\\mid \\beta)=\\prod\_{i=1}\^n \\frac{1}{y\_i}\\text{exp}\\left(-\\text{e}\^{(\\eta\_i)}(\\text{exp}(\\eta\_i))\^{y\_i}\\right),
\\]
...
\\[
p(y \\mid \\beta)=\\prod\_{i=1}\^n {n\_i \\choose y\_i} {e\^{\\eta\_i} \\choose 1+e\^{\\eta\_i}}\^{y\_i} {1 \\choose 1+e\^{\\eta\_i}}\^{n\_i-y\_i}. 
\\]
