...
\\[
p(\\mu \\mid y) = \\frac{p(\\mu)p(y\\mid \\mu)}{p(y)} = \\frac{1}{\\sqrt{2\\pi\\sigma\^2\_0}}\\frac{1}{\\sqrt{\\left(2\\pi\\sigma\^2\\right)\^n}}\\text{exp}\\left(-\\frac{1}{2} \\left[\\frac{(\\mu - \\mu\_0)\^2}{\\sigma\^2\_0} + \\sum\_{i=1}\^n\\frac{(y\_i-\\mu)\^2}{\\sigma\^2} \\right] \\right).
\\]
