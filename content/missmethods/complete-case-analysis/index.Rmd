...
\\[
\\text{Var}(\\hat{\\theta}\_{cc}) = \\text{Var}(\\hat{\\theta}\_{eff})(1 + \\Delta\_{cc}),
\\]
...
\\[
\\Delta\^{\\star}\_{cc} = \\Delta\_{cc} = \\frac{n - n\_{cc}}{n\_{cc}},
\\]
...
\\[
\\Delta\_{cc} \\approx \\frac{n - n\_{cc}\\rho\^{2}}{n\_{cc}(1 - \\rho\^{2}) + n\_{cc}\\rho\^{2}}.
\\]

$\Delta_{cc}$ varies from zero (when $\rho=0$) to $\Delta^{\star}_{cc}$ as $\rho^{2} \rightarrow 1$. However, for the regression coefficient of $y_2$ on $y_1$ we have that $\Delta\_{cc}=0$ since the incomplete observations of $y_1$ provide no information for the regression of $y_2$ on $y_1$. 
...
\\[
\\mu = \\pi\_{cc}\\mu\_{cc} + (1 - \\pi_{cc}\\mu_{ic}). 
\\]
...
\\[
\\mu\_{cc} - \\mu = (1 - \\pi\_{cc})(\\mu\_{cc} - \\mu\_{ic}).  
\\]
