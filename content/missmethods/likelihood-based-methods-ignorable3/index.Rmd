...
\\[
L\_{full}(\\theta, \\psi \\mid y\_{0},m) = \\int f\\left(y\_{0},y\_{1} \\mid \\theta \\right) f\\left(m \\mid y\_{0},y\_{1}, \\psi \\right)dy\_{1}
\\]
...
\\[
L\_{ign}\\left(\\theta \\mid y\_{0} \\right) = \\int f(y\_{0},y\_{1}\\mid \\theta)dy\_{1},
\\]
...
\\[
\\frac{L\_{full}\\left( \\theta, \\psi \\mid y\_{0}, m \\right)}{L\_{full}\\left( \\theta\^{\\star}, \\psi \\mid y\_{0}, m \\right)}=\\frac{L\_{ign}\\left( \\theta \\mid y\_{0} \\right)}{L\_{ign}\\left( \\theta\^{\\star} \\mid y\_{0}\\right)},
\\]
...
\\[
L\_{full}\\left(\\theta, \\psi \\mid y\_{0},m \\right) = L\_{ign}\\left(\\theta \\mid y\_{0} \\right)  L\_{rest}\\left(\\psi \\mid y\_{0},m \\right)
\\]
...
\\[
f\\left(m \\mid y\_{0}, y\_{1}, \\psi \\right)=f\\left(m \\mid y\_{0}, y\^{\\star}\_{1} \\psi \\right),
\\]

for all $y_{1},y^\star_{1},\psi$. Thus, we have

\\[
f\\left(y\_{0}, m \\mid \\theta, \\psi \\right) = f\\left(m \\mid y\_{0}, \\psi \\right) \\int f\\left(y\_{0},y\_{1} \\mid \\theta \\right)dy\_{1} = f\\left(m \\mid y\_{0}, \\psi \\right) f\\left( y\_{0} \\mid \\theta \\right). 
\\]
...
\\[
p\\left(\\theta, \\psi \\mid y\_{0}, m \\right) \\propto p(\\theta, \\psi) L\_{full}\\left(\\theta, \\psi \\mid y\_{0}, m \\right).
\\]
...
\\[
p(\\theta \\mid y\_{0}) \\propto p(\\theta) L\_{ign}\\left(\\theta \\mid y\_{0} \\right).
\\]
...
\\[
p(\\theta, \\psi \\mid y\_{0}, m) \\propto \\left(p(\\theta)L\_{ign}\\left( \\theta \\mid y\_{0} \\right) \\right) \\left(p(\\psi)L\_{rest}\\left(\\psi \\mid y\_{0},m \\right)  \\right).
\\]
...
\\[
l\_{ign}\\left(\\mu, \\Sigma \\mid y\_{0} \\right) = \\log\\left(L\_{ign}\\left(\\mu,\\Sigma \\mid y\_{0} \\right) \\right) = - \\frac{1}{2}n\_{cc}ln \\mid \\Sigma \\mid - \\frac{1}{2}\\sum\_{i=1}\^{n\_{cc}}(y\_i - \\mu ) \\Sigma\^{-1}(y\_i - \\mu)\^{T} - \\frac{1}{2}(n-n\_{cc})ln\\sigma\_{1} - \\frac{1}{2}\\sum\_{i=n\_{cc}+1}\^{n}\\frac{(y\_{i1}-\\mu\_1)\^2}{\\sigma\_{1}}. 
\\]
