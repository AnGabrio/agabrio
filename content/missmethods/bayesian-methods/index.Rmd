...
\\[
p(\\theta \\mid Y\_0, M) \\equiv p(\\theta \\mid Y\_0) \\propto p(\\theta)f(Y\_0 \\mid \\theta),
\\]
...
\\[
y\_{2i,t+1} \\sim N\\left(\\beta\_{20t} + \\beta\_{21t}y\_{1i}, \\sigma\^2\_{2t}  \\right),
\\]
...
\\[
y\_{1i,t+1} \\sim N\\left(\\beta\_{10t} + \\beta\_{11t}y\_{2i}, \\sigma\^2\_{1t}  \\right),
\\]
...
\\[
x\_{1t+1} \\sim p\\left(x\_1 \\mid x\_{2t},\\ldots,x\_{Jt} \\right),
\\]

\\[
x\_{2t+1} \\sim p\\left(x\_2 \\mid x\_{1t+1},\\ldots,x\_{Jt} \\right),
\\]

up to

\\[
x\_{Jt+1} \\sim p\\left(x\_J \\mid x\_{2t+1},\\ldots,x\_{J-1t+1} \\right).
\\]
...
\\[
Y\^d\_{1t+1} \\sim p\\left(Y\_1 \\mid Y\_0, \\theta\^d\_{t}\\right) \\;\\;\\; \\text{and} \\;\\;\\; \\theta\^d\_{t+1} \\sim p\\left(\\theta \\mid Y\^d\_{1t+1}, Y\_0\\right),
\\]
...
\\[
B=\\frac{T}{D-1}\\sum\_{d=1}\^D(\\bar{\\psi}\_{d.} - \\bar{\\psi}\_{..})\^2, \\;\\;\\; \\text{and} \\;\\;\\; \\bar{V}=\\frac{1}{D}\\sum\_{d=1}\^D s\^2\_{d},
\\]
...
\\[
\\widehat{Var}(\\psi \\mid Y\_0) = \\frac{T-1}{T}\\hat{V} + \\frac{1}{T} B,
\\]
...
\\[
\\sqrt{\\hat{R}} = \\sqrt{\\frac{\\widehat{Var}(\\psi \\mid Y\_0)}{\\hat{V}}},
\\]
