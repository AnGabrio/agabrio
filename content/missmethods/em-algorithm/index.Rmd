...
\\[
L\\left(\\theta \\mid Y\_0 \\right) = \\int f\\left(Y\_0, Y\_1 \\mid \\theta  \\right)dY\_1
\\]
...
\\[
D\_l\\left(\\theta \\mid Y\_0 \\right) \\equiv \\frac{\\partial ln L\\left(\\theta \\mid Y\_0 \\right)}{\\partial \\theta} = 0, 
\\]
...
\\[
Q\\left(\\theta \\mid \\theta\_t \\right) = \\int l\\left(\\theta \\mid Y \\right)f\\left(Y\_0  \\mid Y\_1 , \\theta = \\theta\_t \\right)dY\_0.
\\]

The M step determines $\theta_{t+1}$ by maximising this expected complete data loglikelihood:

\\[
Q\\left(\\theta\_{t+1} \\mid \\theta\_t \\right) \\geq Q\\left(\\theta \\mid \\theta\_t \\right),
\\]
...
\\[
E\\left(\\sum\_{i=1}\^{n}y\_i \\mid \\theta\_t, Y\_0 \\right) = \\sum\_{i=1}\^{n\_{cc}}y\_i + (n-n\_{cc})\\mu\_t 
\\]

and

\\[
E\\left(\\sum\_{i=1}\^{n}y\^2\_i \\mid \\theta\_t, Y\_0 \\right) = \\sum\_{i=1}\^{n\_{cc}}y\^2\_i + (n-n\_{cc})\\left(\\mu\^2\_t + \\sigma\^2\_t \\right) 
\\]
...
\\[
\\mu\_{t+1} = \\frac{E\\left(\\sum\_{i=1}\^n y\_i \\mid \\theta\_t, Y\_0 \\right)}{n}
\\]

and 

\\[
\\sigma\^2\_{t+1} = \\frac{E\\left(\\sum\_{i=1}\^n y\^2\_i \\mid \\theta\_t, Y\_0 \\right)}{n} - \\mu\^2\_{t+1}.
\\]
