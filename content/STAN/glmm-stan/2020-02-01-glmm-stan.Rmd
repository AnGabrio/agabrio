...
categories: ["R", "STAN", "generalised linear mixed models"]
tags: ["tutorials", "STAN", "generalised linear mixed models"]
...
```{r chain_model, eval=FALSE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
#prepare data
y1<-data$y1
y2<-data$y2
...
The diagnostic results are not perfect, perhaps I should run the algorithm a bit longer to get better results as a total of $5000$ iterations does not seem enough. Here I do not bother to save time. To get an idea about posterior results I can plot at the average marginal mean offensive and defensive skills for each of the $12$ teams in the league using the following code.
 
```{r effects, eval=FALSE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag, eval=FALSE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r ppc, eval=FALSE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r results_points, eval=FALSE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
Predicted results are not so bad, especially when looking at the number of league points scored by the teams between the replicated and observed season which are quite similar. These results suggest that the model seems to predict in a reasonable way the league results. To further assess this aspect, we consider two plots. The first compares the ranking of the teams across a large number of replications of the season based on the number of wins, losses and league points gained for each team.

```{r plot1, eval=FALSE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
#summarise pred results
...
```{r plot2, eval=FALSE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
