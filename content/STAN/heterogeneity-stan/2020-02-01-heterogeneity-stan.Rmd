...
categories: ["R", "heterogeneity", "STAN", "heteroskedasticity"]
tags: ["tutorials", "STAN", "heterogeneity", "heteroskedasticity"]
authors: ["Andrea Gabrio"]
bibliography: [citations_stan8.bib]
draft: no
slug: heterogeneity-stan
---
...
```{r data_het_cat1, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
set.seed(126)
...
```{r data_het_cat2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r heterosk_model_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
modelString2 = "
  data {
  int<lower=1> n;
  int<lower=1> nX;
...
```{r data_model_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r chain_model_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
params <- c("beta", "sigma", "log_lik")
nChains = 2
burnInSteps = 500
thinSteps = 1
...
```{r fit_model_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
library(mcmcplots)
...
```{r mcmc_residuals_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
mcmc = as.matrix(data.het.rstan)[, c("beta[1]", "beta[2]")]
# generate a model matrix
newdata = data.frame(x = data.het$x)
Xmat = model.matrix(~x, newdata)
...
```{r mcmc_residuals2_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_residuals3_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
ggplot() + geom_point(data = NULL, aes(y = resid, x = data.het1$x)) + theme_classic()
```

Lets see how well data simulated from the model reflects the raw data.

```{r mcmc_rep_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
mcmc = as.matrix(data.het1.rstan)
# generate a model matrix
Xmat = model.matrix(~x, data.het1)
## get median parameter estimates
...
```{r mcmc_add_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_pv1_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
mcmcpvalue <- function(samp) {
    ## elementary version that creates an empirical p-value for the
    ## hypothesis that the columns of samp have mean zero versus a general
    ## multivariate distribution with elliptical contours.
...
```{r mcmc_post1_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
mcmc = as.matrix(data.het1.rstan)
## Calculate the fitted values
...
```{r mcmc_post2_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_post3_ex2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
## Calculate partial residuals fitted values
