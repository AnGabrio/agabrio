...
categories: ["R", "linear regression", "STAN"]
tags: ["tutorials", "STAN", "linear regression"]
authors: ["Andrea Gabrio"]
bibliography: [citations_stan4.bib]
draft: no
slug: multiple-linear-regression-stan
---
...
```{r center_data, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
data <- within(data, {
...
```{r data_model2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
X = model.matrix(~cx1 * cx2, data = data)
data.list <- with(data, list(Y = y, X = X, nX = ncol(X), n = nrow(data)))
```
...
```{r mcmc_pval2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_mult, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_pv2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_loo, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
## since values are less than zero
library(loo)
...
```{r mcmc_post4, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
library(fields)
...
```{r mcmc_post5, eval=FALSE, echo=FALSE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
mcmc = as.matrix(data.rstan.mult)
## Calculate the fitted values
newdata = expand.grid(cx1 = seq(min(data$cx1, na.rm = TRUE), max(data$cx1,
...
```{r code_modelHP, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
modelStringHP = "
						 data {
...
```{r model_HP_fit, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
X = model.matrix(~cx1 + cx2, data = data)
data.list <- with(data, list(Y = y, X = X, nX = ncol(X), n = nrow(data),
