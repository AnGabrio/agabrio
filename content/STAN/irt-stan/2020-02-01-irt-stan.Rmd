...
categories: ["R", "STAN", "item response theory"]
tags: ["tutorials", "STAN", "item response theory"]
...
different items. The items have binary outcomes, i.e. the items are scored as $1$ if correct and $0$ if not. The $i$-th individual in the sample is assumed to have a latent ability $\theta_i$, and the $i$-th individual's response on the $j$-th item is a random variable $Y_{ij}$ with a Bernoulli distribution. The probability that the $i$-th individual correctly answers the $j$-th item (i.e. the probability that $Y_{ij} = 1$) is assumed to have the following IRF form:
...
```{r stan_code1, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
model1<-"
data {
int<lower=0> n;
int<lower=0> p;
...
library(rstan)
set.seed(3456)
#saveRDS(model1_stan,"model1_stan.rds")
model1_stan<-readRDS("model1_stan.rds")
```
...
```{r stan_code2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
model2<-"
data {
int<lower=0> n;
int<lower=0> p;
...
#saveRDS(model2_stan,"model2_stan.rds")
model2_stan<-readRDS("model2_stan.rds")
```
...
```{r stan_code3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
model3<-"
data {
int<lower=0> n;
int<lower=0> p;
...
#saveRDS(model3_stan,"model3_stan.rds")
model3_stan<-readRDS("model3_stan.rds")
```
