...
categories: ["R", "anova", "STAN", "factor analysis", "factorial designs"]
tags: ["tutorials", "STAN", "anova", "factor analysis", "factorial designs"]
authors: ["Andrea Gabrio"]
bibliography: [citations_stan7.bib]
draft: no
slug: factorial-anova-stan
---
...
```{r interactions1, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
mcmc <- as.matrix(data.rstan)
wch = grep("^beta", colnames(mcmc))
newdata = expand.grid(A = levels(data$A), B = levels(data$B))
...
```{r interactions2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
mcmc <- as.matrix(data.rstan)
wch = grep("^beta", colnames(mcmc))
newdata = expand.grid(A = levels(data$A), B = levels(data$B))
Xmat = model.matrix(~A * B, data = newdata)
