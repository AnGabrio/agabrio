...
```{r dag, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
library(dagitty)
library(ggdag)
library(ggrepel)
library(dplyr)
...
```{r confounders, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
library(tidyverse)
...
```{r colliders, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
set.seed(1234)
...
```{r colliders2, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
...
```{r colliders3, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
...
```{r mediators, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
set.seed(1234)
...
```{r mediators2, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
...
```{r mediators3, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
...
```{r mediators4, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
...
Quick summary of what I have discussed so far:
  ...
  3. Variables that point to both $X$ and $Y$ are confounders that must be controlled for in the analysis. Failing to do so will result in biased statistical results. Mechanisms ($X$ affects $M$ affects $Y$) can be included if you want to shed light on what part of the effect of $X$ on $Y$ is mediated by $M$, but the total effect of $X$ on $Y$ is given by an analysis without $M$. Other types of variables are at best unnecessary, at worst inducing new bias (through collider variables).
