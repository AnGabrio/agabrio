...
```{r dataset_lex, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
set.seed(768)
n <- 40
...
These are only two of many different options that can be customised in regard to the display of tables, figures, code lines colore, size, font, etc.... For a full illustration of the high degree of customisation provided by R markdown I refer to the [dedicated webpage](https://rmarkdown.rstudio.com/). 
...
```{r hist_lex, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE, fig.width=8, fig.height=8}
library(ggplot2)
...
```{r descr_r_lex, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE, results='asis'}
library(knitr)
library(dplyr)
...
```{r corr, echo=TRUE, message=FALSE, warnings=FALSE, eval=FALSE}
#corr coeff
...
```{r regress, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
...
```{r model_res, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
summary(fitm1_r)
```
