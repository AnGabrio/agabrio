...
draft: no
image:
  caption: 'how to deal with ceiling effects in the data'
  focal_point: 'Center'
...
```{r dataset_dgp, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
set.seed(768)
n <- 300
id <- seq(1:n)
...
```{r scatter, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE, fig.width=8, fig.height=8}
...
```{r hist, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE, fig.width=8, fig.height=8}
data_sim_ec$trtf <- factor(data_sim_ec$trt)
levels(data_sim_ec$trtf) <- c("old","new")
...
```{r constant_trick, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
...
```{r two_part_models, echo=TRUE, message=FALSE, warnings=FALSE, eval=TRUE}
