...
where $\lambda=\text{baseline} + \text{skill}_i - \text{skill}_j$. Baseline is the log average number of goals when both teams are equally good. The goal outcome of a match between home team $i$ and away team $j$ is modeled as:
...
```{r mcmc_val1, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_val2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r model_code2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
# model 2
m2_string <- "model {
for(i in 1:n_games) {
  HomeGoals[i] ~ dpois(lambda_home[HomeTeam[i],AwayTeam[i]])
...
```{r chain_model2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_m2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_post, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_post2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_post3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_post4, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r data_check_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
for all different $t$, except the first season which is given an vague prior. Here $\sigma^2_{\text{season}}$ is a parameter estimated using the whole data set. The home and away baselines are given the same kind of priors and below is the resulting `JAGS` model.

```{r model_code3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
# model 3
m3_string <- "model {
for(i in 1:n_games) {
...
And now re-fit the model. These changes to the model unfortunately introduces quit a lot of autocorrelation when running the MCMC sampler. Also, re-define the data list to include information for the season parameters. 

```{r chain_model3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_post_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
dic_m2<-m2.r2jags$BUGSoutput$DIC
...
```{r cat_plot, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
caterplot(team_skill, labels.loc = "above", val.lim = c(0.7, 3.8)) + theme_classic()
```
...
```{r cat_plot2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
n <- nrow(ms3)
...
```{r pred1_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred2_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred3_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred4_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred5_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred6_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred7_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred8_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred9_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pred10_m3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pp1, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
n <- nrow(ms3)
...
```{r pp2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pp3, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r pp4, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
The most likely result is 1 - 1 with a predicted payout of $8.4$ and betsson.com agrees with this also offering their lowest payout for this bet, $5.3$. Not good enough! Looking at the payouts at bettson.com I can see that Sevilla - Valencia: 2 - 0 gives me a payout of $16.0$, that’s much better than my predicted payout of $13.1$. I’ll go for that!
