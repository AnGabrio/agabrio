...
date: 2020-02-13T21:13:14-05:00
...
I will demonstrate logistic regression with a range of possible link functions (each of which yield different parameter interpretations). Consider first the logit function:
...
```{r model_code_v2, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
dat.list <- with(dat, list(y=y, x=x, N=nrow(dat)))
modelString="
model{
  for (i in 1:N) {
...
```{r generate_data_gbin, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
set.seed(876)
...
```{r expl_data1_gbin, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
hist(dat$x)
...
```{r model_code_gbin, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r mcmc_diag_gbin, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
denplot(dat.logit.jags, parms = c("beta0","beta1"))
traplot(dat.logit.jags, parms = c("beta0","beta1"))
...
```{r model2_mcmc_res_gbin, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
...
```{r model2_mcmc_res2_gbin, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
SSres<-apply(Resid^2,1,sum)
...
```{r model_code_v2_trend_gbin, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
summary(as.mcmc(-coefs[,1]/coefs[,2]))
...
```{r model_code_v2_plot_gbin, eval=TRUE, echo=TRUE, comment=NA, warning=FALSE, error=FALSE, message=FALSE}
par(mar = c(4, 5, 0, 0))
